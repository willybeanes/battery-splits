...
import { CatcherLeaderboardRow, SortCol, SortDir } from '@/lib/types'
...
export function CatcherTable({ rows, total, page, pageSize, sortCol, sortDir, onSort, onPage, loading }: Props) {
      ...
      <div className="overflow-x-auto rounded-xl border border-[#e0dbd2]">
        <table className="w-full border-collapse min-w-[760px]">
          ...
          <tbody className={loading ? 'opacity-50' : ''}>
            ...
            {rows.map((row, i) => (
              <tr key={`${row.catcher_id}-${i}`} className="border-b border-[#ece8e1] hover:bg-[#f8f6f2] transition-colors">
                <td className="px-3 py-2.5 text-center text-xs font-mono text-[#bbb]">{startRank + i}</td>
                <td className="px-3 py-2.5 text-left text-sm font-semibold text-[#1a1a1a] whitespace-nowrap">{row.catcher_name}</td>
